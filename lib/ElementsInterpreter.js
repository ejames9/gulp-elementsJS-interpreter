...
var ElementsInterpreter = function(file) {
  ...
  var _parseFileForInlineHTML = function(file) {
    ...
    //Reg Exp's.
    var collectHTMLRE = /(?:(?:<\w+(?:\s?\w+\=["'][\w\-]+['"])*>\s*)+[\w\s!"#$%&'()*+,./:;=?@\^_`{|}~-]*(?:<\/\w+>\s*)*)+<\/\w+>\s/im,
  ...
  var _installNPMPackages = function(dependencies) {
    ...
    dependencies.forEach((package)=> {
      fs.access(process.cwd() + '/node_modules/' + package, fs.F_OK, (err)=> {
        if (err) {
          install.push(package);
          console.log(join(install, ' '));

          if (install.length > 0) {
            log('Installing npm Packages.....', ['red', 'bold']);
            var child = spawn('npm', ['install', '--save', join(install, ' ')], {
              cwd: process.cwd(),
              stdio: 'pipe'
            });
