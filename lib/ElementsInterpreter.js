...
var ElementsInterpreter = function(file) {
  ...
  var _parseFileForElementsSyntax = function(file) {
    ...
    var getElemCommentsRE = /(?:\/\/.*)(?:(?:<(?:['"]?[\.#]?(?:\w+[^\s].?[0-9]?)+<?['"]?=?)+(?:\s?\/>)+)+).*\B/im,
            getElementsRE = /(?:<(?:['"]?[\.#]?(?:\w+[^\s].?[0-9]?)+<?['"]?=?)+(?:\s?\/>)+)+/im,
              getStringRE = /((['"]?[.#]?(\w+-?)+['"]?=?)+[0-9]*[</]?)/im,
        commPlaceholderRE = /\|\|\*-*<\(PLACEHOLDER[0-9]*\)>-*\*\|\|/im,
          commPlaceholder;

        /*This bit of code replaces all comments that contain elementsJS syntax with a placeholder, that will be
        ...
        while (getElementsRE.test(file)) {                //  file = file.toString('utf8');
          tick++
